...
updatedAt: 2019-03-25T03:29:23.049Z
images:
...
<Image1 align="center" width="15%" />

- An edge list is just a list of all of the edges in teh graph. <InlineMath math="edges = (A, B), (A, C),(A, D)..."/>.
...
runs in <InlineMath math="O(|E|)" /> and another runs in <InlineMath math="O(|V|^{3/2})" /> it will depend on the number of
vertices and edges in a particular graph to see which one actually runs faster.

- dense graphs are when the <InlineMath math="|E| \approx |V|^2" />. (each vertex has many edges)
- sparse graphs are when <InlineMath math="|E| /approx |V|"/>. (each vertex has only a few edges)
...
runtime of this algorithm is <InlineMath math="O(|V| + |E|)" /> because in the worst case it will visit each vertex and
