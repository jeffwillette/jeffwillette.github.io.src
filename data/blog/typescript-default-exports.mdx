...
updatedAt: Wed, 09 Jan 2019 02:14:00 GMT
categories:
  - typescript
---

I would often come across the situation where [my editor](https://github.com/mhartington/nvim-typescript) plugin would fail to find
the correct imports for typescript. It would work sometimes and then fail to find imports seemingly randomly. I always though this was
the faul of the plugin but as it turns out it was the underlying TSServer that comes bundled with
[Typescript](https://github.com/Microsoft/TypeScript) that was to blame.

TSServer seemed to work well sometimes and then for no apparent reason it would throw `no-content-provided` errors and the command
`getCodeFixes` would return nothing. The `getCodeFixes` command is useful for automatically adding imports to missing symbols.
I found that TSServer would see that there is an error, but it would fail to find the proper exported symbol even though it was watching
the file that exported it.

As I came to find out, the problem was actually how TSServer was finding the things that I was exporting and the way I was exporting
the symbols themselves.

## Some Background

Following the example [here](https://github.com/mmorearty/tsserver-example) I can send some basic commands to the TSServer and see
the output.
...
I can then send these commands to the TSServer
...
I can see that TSServer saw there were unknown symbols in my file. Commands 3, 4, 5, and 6 send the position of the symbol and the error
code that a fix is wanted for and as you can see they have different results.
...
A plain default exported component was found, but a default exported component that was wrapped in the default export statement
was not found. This was a common pattern that I was using in my projects and it makes the exported symbol invisible to the TSServer.
After reading [this](https://basarat.gitbooks.io/typescript/docs/tips/defaultIsBad.html) and thinking about it. I came to the conclusion
that named exports are almost always better than default exports because they enforce consistent naming and they are more discoverable
by the TSServer.
