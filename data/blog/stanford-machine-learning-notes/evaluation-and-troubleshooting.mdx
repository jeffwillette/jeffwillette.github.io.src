...
updatedAt: 2019-03-14T04:31:37.652Z
categories:
  - Programming
  - Algorithms
  - Machine Learning
...
- try changing <InlineMath math="\lambda" /> values. increasing --> fixes high bias (underfitting). decreasing --> fixes
...
first split the training data into three seperate groups <InlineMath math="J_{\Theta train} \text{   } J_{\Theta cross
validation} J_{\Theta test}"/>.
...
<BlockMath math="
  \begin{aligned}
    ...
    & 2. \quad \theta_0 + x_1\theta_2 + x_1^2\theta_2 + ... + x_n^2\theta_{2n} \\
    & 3. \quad ...
  \end{aligned}
" />
...
- if <InlineMath math="J_{train}(\Theta)" /> is high and <InlineMath math="J_{cv}(\Theta)" /> is also high, there is
  probably an underfitting to the data and we should look to higher polynomial features. This is high bias.
- if <InlineMath math="J_{train}(\Theta)" /> is low and <InlineMath math="J_{cv}(\Theta)" /> is high then that means
