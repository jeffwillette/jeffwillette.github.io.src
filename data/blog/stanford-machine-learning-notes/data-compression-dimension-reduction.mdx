...
updatedAt: 2019-04-19T03:52:56.589Z
categories:
  - Programming
  - Algorithms
  - Machine Learning
...
1. Compute covariance matrix <M i="\Sigma = \frac{1}{m} \sum_{i=1}^n (x^{(i)})(x^{(i)})^T" />. This is done
by multiplying the feature vector by itself transposed, and divided the the constant `m`.
2. Compute the eigenvectors of the matrix <M i="\Sigma"/>. In Octave this can be accomplished by
