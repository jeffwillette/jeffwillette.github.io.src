...
updatedAt: 2019-06-05T01:50:23.351Z
categories:
  - Programming
  - Algorithms
  - Machine Learning
...
1. To reduce the data from `n` dimensions (features) to `k` dimensions... Compute covariance
   matrix <M i="\Sigma = \frac{1}{m} \sum_{i=1}^n (x^{(i)})(x^{(i)})^T = \frac{1}{m} X' * X" />. This is done by
   multiplying the feature vector by itself transposed, and divided the the constant `m` (number of examples).
2. Compute the eigenvectors of the matrix <M i="\Sigma"/>. In Octave this can be accomplished by `[U, S, V] =
...
3. Multiply the reduced U by a training example. <M i="z = U_{reduced}^T * x^{(i)}" />. In this case `U` is an
...
following <M i="X_{approx} = U_{reduce} * z" />. `Ureduced` is an `n x k` and z is a `k x 1`, so the result
...
<M b="1 - \frac{\sum_{(i=1)}^k S_{i,i}}{\sum_{(i=1)}^n S_{i,i}}" />
