...
updatedAt: 2019-03-19T13:59:17.459Z
categories:
  - Programming
  - Algorithms
  - Machine Learning
...
becauseit provides the largest margin between the to sets of data. The other lines are valid, but they don't seem to be
the natural way to draw the boundary between the two sets.

<Image2 width="30%" align="right" /> In order to achieve this, the cost function needs to be changed to not include the lambda term. It instead includes a
`C` regularization term which is multiplied by the cost sum. The <InlineMath math="h_\theta(x)" /> function is also
changed from being the log of the sigmoid to being something different as is shown in the Octave sharts here. The light
...
<Image3 width="30%" align="right" /> being 0 if <InlineMath math="\theta^TX"/> is greater than 1 or less than 1
(for the appropriate term of the cost function <InlineMath math="cost_1 \gg 0 \text{ and } cost_0 \ll 0" />).
...
<BlockMath math="
  _\theta^min C \sum_{i=1}^m \lbrack y^{(i)} cost_1(\theta^T x^{(i)}) + (1 - y) cost_0(\theta^Tx^{(i)}) \rbrack
...
<BlockMath math="f_1 = similarity(x,l^{(1)}) = exp \Big( - \frac{\begin{Vmatrix}x - l^{(1)}\end{Vmatrix}^2}{2\sigma^2} \Big)" />

This makes the value of <InlineMath math="f_1" /> nearly equal to 1 if the feature x is close to the landmark. If the
feature x is far from the landmark, the value oof <InlineMath math="f_1" /> is close to 0.

- With a large <InlineMath math="\sigma" /> value, it is more likely to underfit the data (high bias, low variance, and
  a gradual slope on the 3d graph of the distance)
- With a small <InlineMath mat="\sigma" /> value, it is more likely to overfit the data (lower bias, high variacne and a
...
1. No kernel (linear kernel). predicts `y = 1` when <InlineMath math="\theta^TX >= 0" />. This is good when there are a
...
- if n is mall relative to to m and m is intermediate, use an SVM with a Gaussian kernel.
