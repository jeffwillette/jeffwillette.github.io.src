...
updatedAt: 2019-04-17T14:23:37.957Z
categories:
  - Programming
  - Algorithms
  - Machine Learning
...
that would be the line of best fit for the data.  <InlineMath math="\check{y}_i" /> in the cost function
formula is shorthand for <InlineMath math="h_{0}(x) \text{ or } \theta_{0} + \theta_{1}x" />
...
The cost function can be looked at in respect to <InlineMath math="\theta_{0} \text{ or } \theta_{1}" /> on a 2-dimensional
graph. The y-axis will be the cost function of <InlineMath math="\theta_{i}" /> which is annotated
with <InlineMath math="J(\theta_i)" /> and the x-axis will be <InlineMath math="\theta_{i}" />. In the chart above, the
...
a function of both <InlineMath math="\theta_1 \text{ and } \theta_0" /> so we must express it as such which is
...
the <InlineMath math="\alpha" /> is the size step that is taken for each step. The step is multiplied by the derivative
...
evens out to 0 meaning that the minimum is found and the next step is just <InlineMath math="\theta_j - 0" /> or static.
...
following partial derivatives with respect to <InlineMath math="\theta_0 \text{and} \theta_1" />. The partial
derivative with respect to <InlineMath math="\theta_{0}" /> makes the term with <InlineMath math="\theta_1 x_1" />
...
When the derivative is taken with respect to <InlineMath math="\theta_{0}" /> then <InlineMath math="\theta_0" />
