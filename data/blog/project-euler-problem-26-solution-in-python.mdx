...
updatedAt: 2019-01-21T16:08:42.795Z
categories:
  - Programming
  - Project Euler
  - Python
...
<InlineMath math="1/2=0.5, 1/2=0.(3), 1/4=0.25, 1/5=0.2, 1/6=0.1(6), 1/7=0.(142857), 1/8=0.125, 1/9=0.(1), 1/10=0.1" />

Where <InlineMath math="0.1(6)" /> means <InlineMath math="0.166666..." />, and has a 1-digit recurring cycle.
It can be seen that <InlineMath math="\frac{1}{7}" /> has a 6-digit recurring cycle. Find the value of <InlineMath math="d < 1000" />
for which <InlineMath math="\frac{1}{d}" /> contains the longest recurring cycle in its decimal fraction part.

## My Solution

Thinking about repeating decimals, I can see that what makes things repeat is having the same remainder. for instance
take <InlineMath children="\frac{1}{3}" /> as an example

- <InlineMath math="3\overline{)1}" /> doesn't work, so we move the decimal over one place
- <InlineMath math="3\overline{)10}" /> 3 goes into 10 three times, so we multiply <InlineMath math="3x3" /> and we are left with a
remainder of one, move over one decimal
- It repeats forever because the remainder will always be one and nothing else.

For the case of <InlineMath math="\frac{1}{7}" />

- <InlineMath math="\stackrel{0.142857}{7\overline{)1.00000}}" /> each step in this division generates a remainder
- the set of remainders is <InlineMath math="3, 2, 6, 4, 5, 1, 3..." />
